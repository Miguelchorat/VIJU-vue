...
export default {
  data() {
    return {
      results: null,
      API_REVIEW: "http://127.0.0.1:3001/api/v1/reviews/" + this.$route.params.id
    }
  },
  mounted() {
    this.callAPI()
  ...
  <main v-if="results" class="main main--review">
    <header class="main__header main__header--review">
      ...
      <p class="main__header__score"
        :class="results.score < 2.5 ? 'main__header__score__red' : (results.score >= 2.5 && results.score < 3.5) ? 'main__header__score__yellow' : 'main__header__score__green'">
        {{ results.score }}</p>
